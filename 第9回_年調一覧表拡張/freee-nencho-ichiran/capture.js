/*
 * freee人事労務 年調一覧表ボタン - APIキャプチャ（MAINワールド）
 ...
 * （freee-shinkoku-support の capture.js と同方式）
 */
(function () {
  'use strict';

  var SOURCE = 'freee-nencho-ichiran';
  var MAX_BODY = 400000; // 1レスポンスあたりの保持上限（文字数）。従業員一覧は大きくなりがちなので申告側より広め
  ...
  XMLHttpRequest.prototype.open = function (method, url) {
    this.__fnl_method = method;
    this.__fnl_url = url;
    return origOpen.apply(this, arguments);
  };
  XMLHttpRequest.prototype.send = function () {
    var xhr = this;
    if (shouldCapture(xhr.__fnl_url)) {
      xhr.addEventListener('load', function () {
        try {
          var ct = xhr.getResponseHeader('content-type') || '';
          if (ct.indexOf('json') === -1) return;
          if (xhr.responseType && xhr.responseType !== 'text') return;
          post(makeEntry(xhr.__fnl_method, xhr.__fnl_url, xhr.status, ct, xhr.responseText));
        } catch (e) { /* 観測失敗は無視 */ }
      });
    }
